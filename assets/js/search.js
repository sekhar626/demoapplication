function search(){
    ...
    return {searchItem,categoryType}
}
...
function fetchBooks(url,searchItem) {
    ...
    fetch(url, options)
        ...
        .then(function (jsonData) {
            ...
            let partsDetails = jsonData.filter(each=>each.name.toLowerCase().includes(searchItem))
...
const outres=search()
let url
if(outres.categoryType==='All categories'){
    url='http://localhost:8081/allcategories?'
    console.log(url);
}else{
    url = `http://localhost:8081/allcategories?category_type=${outres.categoryType}`
    console.log(url);
}
fetchBooks(url,outres.searchItem)
